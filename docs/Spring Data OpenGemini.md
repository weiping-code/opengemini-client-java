...
```java
public class OpenGeminiTemplate<T> {

  /**
  * 为数据库名、保留策略名、表名固定的表，提供便捷的操作，封装POJO与数据库记录的转换
  */
  void write(T t);
  ...
  /**
  * 为数据库名、保留策略名、表名需动态变化的表，提供便捷的操作，封装POJO与数据库记录的转换
  */
  boolean isDatabaseExist(String database);
  void createDatabase(String database);

  boolean isRetationPolicyExist(String retentionPolicy);
